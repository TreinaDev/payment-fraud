...
describe 'Payment methods management' do
  context 'GET payment methods' do
    it 'should render all payment methods' do
      ...
      pm.icon.attach(io: File.open('spec/support/storage/mastercard_icon.svg'), filename: 'mastercard_icon.svg')
      ...
      expect(json_response.first[:icon][:blob][:filename]).to eq('mastercard_icon.svg')
      ...
      expect(json_response.last[:icon][:blob][:filename]).to eq('boleto.svg')
    end
